...
const transactionOperations = [
  ...
  },
  {
    type: "sortTable",
    description:
      "Stably sort the body rows of a table by one or more sheet column indexes, placing blank sort values after nonblank values. valueMode raw sorts stored input; display sorts evaluated typed values.",
  },
  {
...
const guideResource = {
  ...
  usageConventions: [
    ...
    "Evaluated display reads include arithmetic, comparisons, text operators, same-sheet and cross-sheet ranges, reference intersection, parenthesized reference union, table structured references such as [@Score], Table1[Amount], 'Table 1'[Amount], [@[Q1]:[Q4]], and Table1[[#Data],[Amount]], core worksheet functions including LN, date/time functions such as TODAY, NOW, DATE, YEAR, MONTH, and DAY, and lookup functions such as INDEX, MATCH, XLOOKUP, and VLOOKUP.",
    "Current formula exclusions include absolute references with $, defined names, LET, totals/header table sections, and unparenthesized comma-as-union inside function argument lists.",
