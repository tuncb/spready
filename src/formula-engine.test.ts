...
test("evaluateSheet supports core math, logical, and text functions over same-sheet values", () => {
  ...
  assert.deepEqual(
    [
      ...
      getCellEvaluation(snapshot, 1, 11).display,
      getCellEvaluation(snapshot, 1, 12).display,
      getCellEvaluation(snapshot, 1, 13).display,
      getCellEvaluation(snapshot, 1, 14).display,
    ],
    [
      "5",
      "12.35",
