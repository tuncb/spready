...
import {
  openAppAndWaitForControlTarget,
  type McpStartupOptions,
} from "./mcp-startup";
import {
  resolveControlTarget,
  SpreadyControlClient,
  type ControlTarget,
} from "./control-client";
...
export class McpControlConnection extends EventEmitter {
  ...
  constructor(
    ...
    this.#createClient = dependencies.createClient ?? ((target) => new SpreadyControlClient(target));
  ...
  requireConnectedClient() {
    if (!this.#client || this.#state !== "connected") {
      throw new Error(
        "Spready app is not connected. Call open_spready_app first, or start Spready and call connect_spready_app.",
      );
    }
  ...
  async openAppAndConnect(): Promise<ConnectionResult> {
    ...
    try {
      ...
      // Fall through to launching a fresh app. The launch failure, if any, becomes the public error.
    }
