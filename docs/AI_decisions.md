...
- Table writes use shared transaction operations (`addTable`, `renameTable`, `resizeTable`, `deleteTable`, `sortTable`, `setTableColumnHighlightRules`) so UI, TCP, and MCP use the same workbook rules.
- `sortTable` stably sorts table body rows only. Header rows remain fixed, blank sort values are placed after nonblank values, and cell styles inside the sorted table columns move with their source cells.
- Table column threshold highlights are persisted as table metadata and rendered as derived UI styling so automation can configure highlights without mutating per-cell styles. Multiple rules can target one column; matching rule styles merge in order.
